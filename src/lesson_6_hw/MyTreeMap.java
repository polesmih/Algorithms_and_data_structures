...
public class MyTreeMap<K extends Comparable<K>, V> {
        private Node root;
        private class Node {
            int height; // добавление высоты узла
            K key;
            V value;
            Node left;
            Node right;
        ...
        public Node(K key, V value, int size, int height) { // изменен конструктор
                this.key = key;
                this.value = value;
                //size = 1;
                this.size = size;
...
//delete(key);
        ...
        private Node put(Node node, K key, V value) {
            if (node == null) {
                return new Node(key, value, 1, 0); // добавлены условия
            }
            int cmp = key.compareTo(node.key);
            if (cmp == 0) {
                node.value = value;
